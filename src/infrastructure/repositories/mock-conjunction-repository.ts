...
const EVENTS: ConjunctionEvent[] = [
  ConjunctionEvent.create({
    ...
    tcpa: TimeToClosestApproach.create(new Date(Date.now() + 2 * 3600_000)),
  }),
  ConjunctionEvent.create({
    ...
    tcpa: TimeToClosestApproach.create(new Date(Date.now() + 6 * 3600_000)),
  }),
  ConjunctionEvent.create({
    objectA: sat(20580, 'HUBBLE', SatelliteObjectType.OPERATIONAL_SATELLITE, L1_ISS, L2_ISS),
    objectB: sat(44715, 'STARLINK-1009', SatelliteObjectType.OPERATIONAL_SATELLITE, L1_SL1, L2_SL1),
    pc: ProbabilityOfCollision.create(5e-6),
    missDistance: MissDistance.create(18000),
    tcpa: TimeToClosestApproach.create(new Date(Date.now() + 12 * 3600_000)),
  }),
]
